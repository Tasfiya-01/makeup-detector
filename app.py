...
st.markdown("""
    ...
    * {font-family: 'Poppins', sans-serif;}
    ...
    .item-pill {
        display: inline-flex;
        align-items: center;
        background: linear-gradient(90deg, #FF6B9D, #C44FD8);
        color: white;
        border-radius: 25px;
        padding: 8px 18px;
        margin: 5px;
        font-size: 0.95rem;
        font-weight: 500;
        box-shadow: 0 3px 10px rgba(196,79,216,0.3);
    }
                    ...
                    with c1:
                        st.markdown(f"""
                            <div class="stat-box">
                                ...
                                <div class="stat-lab">Items Found</div>
                            </div>
                        """, unsafe_allow_html=True)
                    with c2:
                        st.markdown(f"""
                            <div class="stat-box">
                                <div class="stat-num">{len(item_counts)}</div>
                    ...
                    with c3:
                        st.markdown(f"""
                            <div class="stat-box">
                                ...
                                <div class="stat-lab">Avg Confidence</div>
                            </div>
                        """, unsafe_allow_html=True)
                    ...
                    for class_name, conf_score in detections:
                        st.markdown(f"""
                            <div class="detection-item">
                                <span class="det-name">💄 {class_name}</span>
                                <span class="det-conf">{conf_score:.1f}%</span>
                            </div>
                        """, unsafe_allow_html=True)
                else:
                    st.markdown("""
                        <div class="no-detection">
                            😕 No makeup products detected!<br>
                            <small>Try lowering the sensitivity slider</small>
                        </div>
                    """, unsafe_allow_html=True)
    else:
        st.markdown("""
